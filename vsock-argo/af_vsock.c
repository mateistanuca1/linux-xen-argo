...
static int vsock_auto_bind(struct vsock_sock *vsk) {
    ...
    if (vsock_addr_bound(&vsk->local_addr)) return 0;
...
void vsock_remove_bound(struct vsock_sock *vsk) {
    spin_lock_bh(&vsock_table_lock);
    if (__vsock_in_bound_table(vsk)) __vsock_remove_bound(vsk);
    spin_unlock_bh(&vsock_table_lock);
}
...
void vsock_remove_connected(struct vsock_sock *vsk) {
    spin_lock_bh(&vsock_table_lock);
    if (__vsock_in_connected_table(vsk)) __vsock_remove_connected(vsk);
    spin_unlock_bh(&vsock_table_lock);
}
...
void vsock_for_each_connected_socket(struct vsock_transport *transport,
    ...
    for (i = 0; i < ARRAY_SIZE(vsock_connected_table); i++) {
        struct vsock_sock *vsk;
        list_for_each_entry(vsk, &vsock_connected_table[i], connected_table) {
            if (vsk->transport != transport) continue;
...
static bool vsock_use_local_transport(unsigned int remote_cid) {
    if (!transport_local) return false;

    if (remote_cid == VMADDR_CID_LOCAL) return true;
 ...
 * vsk->remote_addr is set (e.g. during the connect() or when a connection
 * request on a listener socket is received). The vsk->remote_addr is used to
...
int vsock_assign_transport(struct vsock_sock *vsk, struct vsock_sock *psk) {
    ...
    if (vsk->transport) {
        if (vsk->transport == new_transport) return 0;
...
bool vsock_find_cid(unsigned int cid) {
    if (transport_g2h && cid == transport_g2h->get_local_cid()) return true;

    if (transport_h2g && cid == VMADDR_CID_HOST) return true;

    if (transport_local && cid == VMADDR_CID_LOCAL) return true;

    if (transport_dgram) return true;
...
static struct sock *vsock_dequeue_accept(struct sock *listener) {
    ...
    if (list_empty(&vlistener->accept_queue)) return NULL;

    vconnected = list_entry(vlistener->accept_queue.next, struct vsock_sock,
                            accept_queue);
...
static int vsock_send_shutdown(struct sock *sk, int mode) {
    ...
    if (!vsk->transport) return -ENODEV;
...
static int __vsock_bind_connectible(struct vsock_sock *vsk,
    ...
    if (!port) port = get_random_u32_above(LAST_RESERVED_PORT);
    ...
    if (addr->svm_port == VMADDR_PORT_ANY) {
        ...
        for (i = 0; i < MAX_PORT_RETRIES; i++) {
            if (port <= LAST_RESERVED_PORT) port = LAST_RESERVED_PORT + 1;
...
static int __vsock_bind_stream(struct vsock_sock *vsk,
    ...
    return vsk->transport->stream_bind(vsk, addr);
}
...
static int __vsock_bind(struct sock *sk, struct sockaddr_vm *addr) {
    ...
    if (vsock_addr_bound(&vsk->local_addr)) return -EINVAL;
...
static struct sock *__vsock_create(struct net *net, struct socket *sock,
                                   struct sock *parent, gfp_t priority,
                                   unsigned short type, int kern) {
    struct sock *sk;
...
s64 vsock_stream_has_data(struct vsock_sock *vsk) {
    if (WARN_ON(!vsk->transport)) return 0;
...
static int vsock_bind(struct socket *sock, struct sockaddr *addr,
                      int addr_len) {
    int err;
    struct sock *sk;
    ...
    if (vsock_addr_cast(addr, addr_len, &vm_addr) != 0) return -EINVAL;
...
static int vsock_getname(struct socket *sock, struct sockaddr *addr, int peer) {
    int err;
    struct sock *sk;
    struct vsock_sock *vsk;
    struct sockaddr_vm *vm_addr;
...
static int vsock_shutdown(struct socket *sock, int mode) {
    ...
    if (sock->state == SS_UNCONNECTED) {
        err = -ENOTCONN;
        if (sock_type_connectible(sk->sk_type)) goto out;
    } else {
...
static __poll_t vsock_poll(struct file *file, struct socket *sock,
    ...
    if (sk->sk_shutdown & RCV_SHUTDOWN || vsk->peer_shutdown & SEND_SHUTDOWN) {
        mask |= EPOLLRDHUP;
    }

    if (sk_is_readable(sk)) mask |= EPOLLIN | EPOLLRDNORM;
    ...
    if (sock->type == SOCK_DGRAM) {
         ...
         * queue can be read.
         */
        if (sk->sk_state == TCP_LISTEN && !vsock_is_accept_queue_empty(sk))
            mask |= EPOLLIN | EPOLLRDNORM;
        ...
        if (transport && transport->stream_is_active(vsk) &&
            ...
            int ret = transport->notify_poll_in(vsk, target, &data_ready_now);
            if (ret < 0) {
                mask |= EPOLLERR;
            } else {
                if (data_ready_now) mask |= EPOLLIN | EPOLLRDNORM;
            }
        }
        ...
        if (transport && sk->sk_state == TCP_ESTABLISHED) {
            if (!(sk->sk_shutdown & SEND_SHUTDOWN)) {
                ...
                int ret = transport->notify_poll_out(vsk, 1, &space_avail_now);
                if (ret < 0) {
                    mask |= EPOLLERR;
                } else {
                    if (space_avail_now)
...
static int vsock_dgram_sendmsg(struct socket *sock, struct msghdr *msg,
     ...
     */
    if (msg->msg_name &&
        vsock_addr_cast(msg->msg_name, msg->msg_namelen, &remote_addr) == 0) {
    ...
    if (!transport->dgram_allow(remote_addr->svm_cid, remote_addr->svm_port)) {
        err = -EINVAL;
        goto out;
    }
...
static int vsock_dgram_connect(struct socket *sock, struct sockaddr *addr,
...
int __vsock_dgram_recvmsg(struct socket *sock, struct msghdr *msg, size_t len,
                          int flags) {
    struct sock *sk = sock->sk;
    struct vsock_sock *vsk = vsock_sk(sk);
...
int vsock_dgram_recvmsg(struct socket *sock, struct msghdr *msg, size_t len,
                        int flags) {
#ifdef CONFIG_BPF_SYSCALL
    struct sock *sk = sock->sk;
    ...
    if (prot != &vsock_proto) return prot->recvmsg(sk, msg, len, flags, NULL);
#endif
...
static int vsock_transport_cancel_pkt(struct vsock_sock *vsk) {
    ...
    if (!transport || !transport->cancel_pkt) return -EOPNOTSUPP;
...
static void vsock_connect_timeout(struct work_struct *work) {
    ...
    lock_sock(sk);
    if (sk->sk_state == TCP_SYN_SENT && (sk->sk_shutdown != SHUTDOWN_MASK)) {
        sk->sk_state = TCP_CLOSE;
...
static int vsock_connect(struct socket *sock, struct sockaddr *addr,
    ...
    switch (sock->state) {
        ...
        default:
             ...
             * endpoints.
             */
            if (!transport || !transport->stream_allow(remote_addr->svm_cid,
                                                       remote_addr->svm_port)) {
                err = -ENETUNREACH;
                goto out;
            }
    ...
    while (sk->sk_state != TCP_ESTABLISHED && sk->sk_err == 0) {
        ...
        if (signal_pending(current)) {
            ...
            sk->sk_state =
                sk->sk_state == TCP_ESTABLISHED ? TCP_CLOSING : TCP_CLOSE;
            sock->state = SS_UNCONNECTED;
            vsock_transport_cancel_pkt(vsk);
            vsock_remove_connected(vsk);
            goto out_wait;
...
static int vsock_accept(struct socket *sock, struct socket *newsock,
    ...
    if (listener->sk_err) err = -listener->sk_err;
...
static void vsock_update_buffer_size(struct vsock_sock *vsk,
                                     const struct vsock_transport *transport,
                                     u64 val) {
    if (val > vsk->buffer_max_size) val = vsk->buffer_max_size;

    if (val < vsk->buffer_min_size) val = vsk->buffer_min_size;

    if (val != vsk->buffer_size && transport && transport->notify_buffer_size)
        transport->notify_buffer_size(vsk, &val);

    vsk->buffer_size = val;
}

static int vsock_connectible_setsockopt(struct socket *sock, int level,
    ...
    if (level != AF_VSOCK && level != SOL_SOCKET) return -ENOPROTOOPT;
    ...
    switch (optname) {
        ...
        case SO_VM_SOCKETS_CONNECT_TIMEOUT_OLD: {
            ...
            if (tv.tv_sec >= 0 && tv.tv_usec < USEC_PER_SEC &&
                ...
                vsk->connect_timeout =
                    tv.tv_sec * HZ + DIV_ROUND_UP((unsigned long)tv.tv_usec,
                                                  (USEC_PER_SEC / HZ));
...
static int vsock_connectible_getsockopt(struct socket *sock, int level,
    ...
    if (level != AF_VSOCK) return -ENOPROTOOPT;
    ...
    if (len < lv) return -EINVAL;
    if (len > lv) len = lv;
    if (copy_to_user(optval, &v, len)) return -EFAULT;
...
static int vsock_connectible_sendmsg(struct socket *sock, struct msghdr *msg,
    ...
    if (sk->sk_shutdown & SEND_SHUTDOWN || vsk->peer_shutdown & RCV_SHUTDOWN) {
        err = -EPIPE;
        goto out;
    }
    ...
    if (msg->msg_flags & MSG_ZEROCOPY && !vsock_msgzerocopy_allow(transport)) {
        err = -EOPNOTSUPP;
        goto out;
    }
    ...
    while (total_written < len) {
        ...
        if (sk->sk_type == SOCK_SEQPACKET) {
            written =
                transport->seqpacket_enqueue(vsk, msg, len - total_written);
        } else {
            written = transport->stream_enqueue(vsk, msg, len - total_written);
        }
        ...
        err = transport->notify_send_post_enqueue(vsk, written, &send_data);
        if (err < 0) goto out_err;
    }
...
static int vsock_connectible_wait_data(
    struct sock *sk, struct wait_queue_entry *wait, long timeout,
    struct vsock_transport_recv_notify_data *recv_data, size_t target) {
    const struct vsock_transport *transport;
    struct vsock_sock *vsk;
    s64 data;
    int err;
...
static int __vsock_stream_recvmsg(struct sock *sk, struct msghdr *msg,
    ...
    while (1) {
        ...
        err =
            vsock_connectible_wait_data(sk, &wait, timeout, &recv_data, target);
        if (err <= 0) break;

        err = transport->notify_recv_pre_dequeue(vsk, target, &recv_data);
        if (err < 0) break;
        ...
        err = transport->notify_recv_post_dequeue(
            vsk, target, read, !(flags & MSG_PEEK), &recv_data);
        if (err < 0) goto out;

        if (read >= target || flags & MSG_PEEK) break;
...
static int __vsock_seqpacket_recvmsg(struct sock *sk, struct msghdr *msg,
    ...
    if (sk->sk_err) {
         ...
         */
        if (msg_len > len) msg->msg_flags |= MSG_TRUNC;
    }
...
static int vsock_set_rcvlowat(struct sock *sk, int val) {
    ...
    if (val > vsk->buffer_size) return -EINVAL;
...
static int vsock_create(struct net *net, struct socket *sock, int protocol,
                        int kern) {
    struct vsock_sock *vsk;
    struct sock *sk;
    int ret;
...
static long vsock_dev_do_ioctl(struct file *filp, unsigned int cmd,
                               void __user *ptr) {
    u32 __user *p = ptr;
    ...
    switch (cmd) {
        case IOCTL_VM_SOCKETS_GET_LOCAL_CID:
            ...
            if (put_user(cid, p) != 0) retval = -EFAULT;
            break;
...
static long vsock_dev_ioctl(struct file *filp, unsigned int cmd,
                            unsigned long arg) {
    return vsock_dev_do_ioctl(filp, cmd, (void __user *)arg);
}

#ifdef CONFIG_COMPAT
static long vsock_dev_compat_ioctl(struct file *filp, unsigned int cmd,
                                   unsigned long arg) {
...
void vsock_core_unregister(const struct vsock_transport *t) {
    ...
    if (transport_h2g == t) transport_h2g = NULL;

    if (transport_g2h == t) transport_g2h = NULL;

    if (transport_dgram == t) transport_dgram = NULL;

    if (transport_local == t) transport_local = NULL;
